@php 
	$total_resorts = count(__('special-offer/'.$slug.'.resorts'));
	$resort_id[0]=1;
	$resort_ihotelier[0]=1;
	$tipo[0]=1;
	$contador=0;
	$indice=0;
	$minimo = __('special-offer/'.$slug.'.minimo_noches');
	$code= __('specials/'.$slug.'.code');

	for($m=0;$m<$total_resorts;$m++){
			$resort_id[$m]=__('special-offer/'.$slug.'.resorts.'.$m.'.resort_id');
			$resort_ihotelier[$m]=__('special-offer/'.$slug.'.resorts.'.$m.'.ihotelier_id');
			$tipo[$m]=__('special-offer/'.$slug.'.resorts.'.$m.'.tipo');
	}
@endphp
