		...
		<div class="row">
			<div class="w-100">
				<div class="container booking-interno-container">
					...
					@php
						$total=count(__('resorts-booking.'.$destino));
						$label_destino="Punta Cana"; $rh='111';
					@endphp
					@switch($destino)
					    @case('cancun')
					        @php $label_destino="Cancun";@endphp
					        @break
					    @case('playa-del-carmen')
					        @php $label_destino="Playa del Carmen";@endphp
					        @break
						@case('riviera-maya')
					        @php $label_destino="Riviera Maya"; @endphp
					        @break
					    @case('st-maarten')
					        @php $label_destino="Sint Maarten"; @endphp
					        @break
					    @case('curacao')
					        @php $label_destino="Curacao"; @endphp
					        @break
					    @default
					        @php $label_destino="Punta Cana";$rh='0'; @endphp
					@endswitch
								...
								<div class="je-tab-content yellow">
									<div class="booking-interno-inputs">
										...
										<select class="form-control combo" name="hotelId" id="hotelId" onchange="changeResort();">
											<optgroup label="{{$label_destino}}">
											@for($i=0;$i<$total;$i++)
												<option value="{{__('resorts-booking.'.$destino.'.'.$i.'.ihotelier')}}">{{__('resorts-booking.'.$destino.'.'.$i.'.name')}}</option>
											@endfor
											</optgroup>
										</select>
									</div>
						...
						<form name="booking_engine_form_rh" id="booking_engine_form" action="" onsubmit="return validaForm();" target="_blank">
								<div class="je-tab-content sky-blue">
									<div class="booking-interno-inputs w50">
										...
										<select class="form-control combo" name="hotel" id="hotel" onChange="">
											<optgroup label="{{$label_destino}}">
											@for($i=0;$i<$total;$i++)
												<option value="{{__('resorts-booking.'.$destino.'.'.$i.'.reservhotel')}}">{{__('resorts-booking.'.$destino.'.'.$i.'.name')}}</option>
											@endfor
											</optgroup>
										</select>
									</div>
